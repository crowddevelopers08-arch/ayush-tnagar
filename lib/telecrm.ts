...
const FALLBACK_PAGE = "https://tnagar.ayushortho.in/"
...
export async function sendToTeleCRM(lead: TeleCRMLead): Promise<TeleCRMResult> {
  ...
  const branch = lead.branch || "T. Nagar"
  ...
  const payload = {
    ...
    actions: [
      ...
      { type: "SYSTEM_NOTE", text: `Where it hurts: ${lead.area || "Not specified"}` },
